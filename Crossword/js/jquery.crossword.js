...
var saizu=0;
(function($){
	$.fn.crossword = function(entryData, option) {
		console.log('funcStart');
			...
			*/
			...
			puzz.data = entryData;
			/*var ob= '<div id="puzzle-clues">' + 
					'<h2>Horizontal</h2>'+
					'<img id="img1" class="img-fluid rounded" src="imagenes/img11.png" height="30%" width="30%">'+
					'<img id="img2" class="img-fluid rounded" src="imagenes/img12.png" height="30%" width="30%">'+
					'<img id="img3" class="img-fluid rounded" src="imagenes/img13.png" height="30%" width="30%">'+
					'<ol id="across">'+
					'</ol><h2>Vertical</h2>'+
					'<img id="img4" class="img-fluid rounded" src="imagenes/img14.png" height="30%" width="30%">'+
					'<img id="img5" class="img-fluid rounded" src="imagenes/img15.png" height="30%" width="30%">'+
					'<img id="img6" class="img-fluid rounded" src="imagenes/img16.png" height="30%" width="30%">'+
					'<ol id="down"></ol></div>';
			...
			buildEntries: function() {
					...
					for (var x=1, p = entryCount; x <= p; ++x) {
						...
						for (var i=0; i < entries[x-1].length; ++i) {
							...
							//console.log('Leyendo: '+ entries[x-1][i]);
							...
							if(x > 1 ){
								if (puzz.data[x-1].position === puzz.data[x-2].position) {
									//console.log(puzz.data[x-1].position +'/'+puzz.data[x-2].position);
									hasOffset = true;
								};
							}
							...
							if($(light).empty()){
								$(light)
									...
									//console.log('Empty: ' + entries[x-1][i]);
									//console.log('BSpan: ' + entries[x-1][i]);
									//console.log('BSpans: ' + puzz.data[x-1].position);
									if (i == 0) {
										$(light).append('<span>' + puzz.data[x-1].position + '</span>');
										//console.log('Span: ' + entries[x-1][i]);
										//console.log('Spans: ' + puzz.data[x-1].position);
									}
							}
					...
					for (var i=0; i < entryCount; ++i) {
						//console.log('ho'+puzz.data[i].position);
						$groupedLights = $('.entry-' + i);
						console.log($('.entry-' + i +':eq(0) span').length);
						if(!$('.entry-' + i +':eq(0) span').length){
							var test = puzz.data[i].position-1;
							$groupedLights.eq(0)
								.append('<span>' + test + '</span>');
								//console.log('la'+puzz.data[i].position);
						}
					}	
				...
				*/
				checkAnswer: function(e) {
					...
					if(check[puzz.data[activePosition].position-1]==false){
						if(valToCheck === currVal){	
							...
							solvedToggle = true;
							console.log(check[puzz.data[activePosition].position-1]);
							if(check[puzz.data[activePosition].position-1]==true){
								
							}else{
								check[puzz.data[activePosition].position-1]=true;
								saizu++;
							}
							//saizu++;
							document.getElementById("img"+puzz.data[activePosition].position).src="imagenes/img"+option+puzz.data[activePosition].position+"C.png";
							return;
						}
					}else{
